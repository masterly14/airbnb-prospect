...
export async function POST(request: Request) {
  ...
  const workerUrl =
    process.env.ACCOUNT_REAPER_WORKER_URL ??
    (process.env.WORKER_BASE_URL
      ? `${process.env.WORKER_BASE_URL.replace(/\/$/, '')}/run/account-reaper`
      : undefined)
  ...
  return NextResponse.json({
    ok: true,
    source: auth.source,
    message: 'Cron verified. Run npm run account-reaper on the Playwright worker.',
  })
}
